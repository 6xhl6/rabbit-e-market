/** SKU 弹出层实例类型 */
export interface SkuPopupInstanceType {
  // === 数据 ===
  /** 组件是否加载完成 */
  complete: boolean
  /** 存放被选中的值 */
  ...
  /** 打开弹出层 */
  open: () => void
  /** 关闭弹出层，source: 'mask' | 'close' */
  close: (source?: string) => void
  /** 主动设置 sku，例如 selectSku({ sku: ["红色","256G"], num: 5 }) */
  selectSku: (obj: { sku: string[]; num?: number }) => void
  /** 初始化 */
  init: (notAutoClick?: boolean) => void
}
